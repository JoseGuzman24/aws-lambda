...
export const getStarWarsAndSave = async (event) => {
    try {
        ...
        const response = await axios.get(
            `https://swapi.py4e.com/api/planets/${number}/`
        );
        const starWarsEntity: StarWarsEntity = new StarWarsEntity({
            ...response.data,
            id: v4(),
        });
        const dynamodb = new DynamoDB.DocumentClient();
        await dynamodb
            .put({ TableName: "StarWarsTable", Item: starWarsEntity })
            .promise();
